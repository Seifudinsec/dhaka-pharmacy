...
const COL_W = ['45%', '75%', '60%', '70%', '65%', '55%', '80%', '50%', '65%', '72%', '58%', '68%'];
      ...
      <thead>
        <tr>
          {Array.from({ length: cols }).map((_, i) => (
            <th key={i} style={{ padding: '12px 16px', textAlign: 'left', borderBottom: '1px solid var(--gray-200)' }}>
              <Bone width={i === 0 ? '50%' : COL_W[i % COL_W.length]} height={12} />
            </th>
          ))}
        </tr>
      </thead>
          ...
          <tr key={r} style={{ borderBottom: '1px solid var(--gray-100)' }}>
            {Array.from({ length: cols }).map((_, c) => (
              <td key={c} style={{ padding: '14px 16px' }}>
                <Bone width={c === cols - 1 ? '55%' : COL_W[(r + c) % COL_W.length]} height={13} />
              </td>
            ))}
          </tr>
        ))}
...
/** Skeleton for a metric/summary card row (Inventory, Dashboard) */
export function SkeletonMetricRow({ count = 4 }) {
  return (
    <div className="inventory-summary-grid" aria-hidden="true">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="metric-card" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 20px' }}>
...
export function SkeletonSummaryCards({ count = 4 }) {
  return (
    <div className="summary-cards" aria-hidden="true">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="summary-card" style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
          <div className="skeleton-shimmer" style={{ width: 48, height: 48, borderRadius: 10, flexShrink: 0 }} />
          <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ padding: '0 4px' }} aria-hidden="true">
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', gap: 6, height: 100 }}>
        {BAR_H.map((h, i) => (
          <div key={i} className="skeleton-shimmer" style={{ flex: 1, height: `${h}%`, borderRadius: '3px 3px 0 0' }} />
        ))}
      </div>
