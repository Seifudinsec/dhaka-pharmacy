...
export default function InventoryPage() {
  ...
  const fetchMedicines = useCallback(
    async (q = debouncedSearch, f = filter) => {
      setLoading(true);
      try {
  ...
  const getExpiryDisplay = (m) => {
    ...
    if (days < 0)
      return (
        <span className="badge badge-red">Expired {Math.abs(days)}d ago</span>
      );
    if (days === 0)
      return <span className="badge badge-red">Expires today</span>;
    if (days <= 30)
      return <span className="badge badge-amber">Exp. in {days}d</span>;
    return (
      <span style={{ fontSize: 13 }}>{format(expiry, "dd MMM yyyy")}</span>
    );
  };
  ...
  const getStockDisplay = (m) => {
    ...
    if (m.stock < 10)
      return <span className="badge badge-amber">{m.stock} left (low)</span>;
  ...
  const summary = {
    ...
    expired: medicines.filter((m) => new Date(m.expiryDate) < new Date())
      .length,
  };
    ...
    <div>
      ...
      <div className="card">
        <div className="card-header">
          <h2>
            <AppIcon icon={faBoxesStacked} className="header-inline-icon" />{" "}
            Inventory
          </h2>
        </div>
          ...
          {showLoader ? (
            ...
            <div className="empty-state">
              <div className="empty-icon">
                <AppIcon icon={faBoxesStacked} size="xl" tone="muted" />
              </div>
            ...
            <table>
              ...
              <tbody>
                {medicines.map((m, i) => (
                  <tr
                    key={m._id}
                    className={`${getRowClass(m)} ${highlightId === m._id ? "inventory-row-highlight" : ""}`.trim()}
                  >
                    <td
                      data-label="#"
        ...
        <div className="inventory-legend">
          <span className="inventory-legend-chip danger">■</span> Expired / Out
          of stock
          <span className="inventory-legend-chip warning">■</span> Low stock /
