...
export default function SalesPage() {
  ...
  const openReturnModal = (sale) => {
    ...
    const returnItemsData = sale.items.map((item) => {
      ...
      console.log("Item medicine data:", {
  ...
  const updateReturnQuantity = (index, quantity) => {
    ...
    newReturnItems[index].returnQuantity = Math.min(
      Math.max(0, quantity),
      maxReturnable,
    );
  ...
  const processReturn = async () => {
    ...
    for (const item of itemsToReturn) {
      if (item.returnQuantity > item.maxReturnable) {
        toast.error(
          `Cannot return ${item.returnQuantity} units of ${item.medicineName}. Only ${item.maxReturnable} units available.`,
        );
        return;
      }
    }
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `Return processed successfully! ${itemsToReturn.length} item(s) returned. Stock updated.`,
        );
  ...
  return (
    <div>
      {showLoader ? (
        <SkeletonMetricRow count={2} gridClassName="sales-summary-grid" />
      ) : (
      ...
      <div className="card">
        <div className="card-header">
          <h2>
            <AppIcon icon={faMoneyBillTrendUp} className="header-inline-icon" />{" "}
            Sales History
          </h2>
        </div>
        <div className="table-toolbar">
          <div className="table-toolbar-filters">
            ...
            <button className="btn btn-primary" onClick={() => fetchSales(1)}>
              <AppIcon icon={faMagnifyingGlass} /> Filter
            </button>
            <button
        ...
        <div className="table-wrap table-responsive-cards">
          {showLoader ? (
            ...
            <div className="empty-state">
              ...
              <p>
                Adjust the date filter or process a sale from the Billing page.
              </p>
            </div>
          ) : (
            <table>
              ...
              <tbody>
                {sales.map((sale) => (
                  <React.Fragment key={sale._id}>
                    <tr>
                      <td data-label="Date" style={{ fontSize: 13 }}>
                      ...
                      <td data-label="Status">
                        <span
                          className={`status-badge ${sale.status || "completed"}`}
                        >
                      ...
                      <td data-label="Actions">
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() =>
                              setExpanded(
                                expanded === sale._id ? null : sale._id,
                              )
                            }
                          >
                            {expanded === sale._id ? (
                              <>
                                <AppIcon icon={faChevronUp} /> Hide
                              </>
                            ) : (
                              <>
                                <AppIcon icon={faChevronDown} /> Details
                              </>
                            )}
                          </button>
                        </div>
          ...
          >
            ...
            <div style={{ display: "flex", gap: 6 }}>
              <button
                ...
                disabled={page <= 1}
              >
                <AppIcon icon={faChevronLeft} /> Prev
              </button>
              <button
                className="btn btn-secondary btn-sm"
                onClick={() => fetchSales(page + 1)}
              ...
              <div className="modal-body">
                ...
                >
                  ...
                  <div style={{ fontSize: "14px", color: "var(--gray-600)" }}>
                    ...
                    - KES {selectedSale.total.toFixed(2)}
                  </div>
                </div>
                    ...
                    {returnItems.map((item, index) => (
                      ...
                      >
                        <div>
                          ...
                          >
                            Original: {item.originalQuantity} units @ KES{" "}
