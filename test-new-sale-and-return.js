...
async function testNewSaleAndReturn() {
  try {
    ...
    const initialDashboardResponse = await axios.get(
    ...
    const updatedDashboardResponse = await axios.get(
      `${API_BASE}/dashboard/stats`,
      {
        headers: { Authorization: `Bearer ${token}` },
      },
    ...
    const updatedMedicine = updatedMedicines.find(
      (m) => m._id === testMedicine._id,
    );

    console.log(
      `Medicine stock before: ${testMedicine.stock}, after: ${updatedMedicine.stock}`,
    );
    ...
    console.log(
      `Sale return transactions: ${updatedSale.returnTransactions.length}`,
    );
